...
It is not advisable to render this Quarto markdown file, as it involves substantial computation - please consider running each code block in sequential order to replicate the paper's results. It is also advised to run each block of this file in "source" editor mode, as specified in the above YAML parameter.
...
# 
# # Install some "twostageSL" package dependencies and packages from GitHub (if not already installed)
...
# library(devtools)
# devtools::install_github("wuziyueemory/twostageSL", dependencies = TRUE)
# devtools::install_github("e-mitchell/meps_r_pkg/MEPS", dependencies = TRUE)
```

```{r, message = FALSE, warning = FALSE, results='hide'}
# Loading relevant packages (assuming they are installed/accessible)
library(grid)
library(Matrix)
library(survival)
library(nnls)
...
# Loading MEPS HC FYC Data Files

Note: the "2023_MEPS_HC_FYC.xlsx" file from this project's GitHub repository MUST be in the same file path as this file to be used in the below code block. It can be found and downloaded directly here: https://github.com/jficorilli/DS_340W_Project/blob/main/Project%20Implementation/2023_MEPS_HC_FYC.xlsx. The remaining MEPS HC FYC files are downloaded using the "read_MEPS()" function from the "MEPS" package provided by the AHRQ, which is available for reference in this GitHub repository: https://github.com/e-mitchell/meps_r_pkg.
