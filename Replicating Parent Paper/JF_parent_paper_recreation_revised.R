...
# Adapted/consolidated from three R code files in first author's GitHub: MEPS_ind.R, twostageSL_ind.R, and Estimators.R. 

# Loading relevant libraries

# For parallel processing (personal addition):
library(parallel)
library(doParallel)
library(foreach)
...
# Initial code for two-stage super learner (From twostageSL_ind.R)
...
# Loading MEPS train data (same as in parent paper's GitHub and used in its MEPS_ind.R) using my project GitHub file paths
...
tryCatch({
  ...
  mse.value <- c(# Individual algorithm
    ...
    # Discrete two-stage SL with indicators
    mean((test$TOTEXP - discrete.pred.twostage.indicator)^2))
  
  mse_df <- data.frame("Algo" = c(twostage.fit$libraryNames, "One-stage Super Learner", "Two-stage Super Learner", 
                                  "Two-stage Super Leaner + Indicator", "Discrete two-stage Learner", "Discrete two-stage Leaner + Indicator"),
                    "MSE" = mse.value)
  ...
  mae_df <- data.frame("Algo" = c(twostage.fit$libraryNames, "One-stage Super Learner", "Two-stage Super Learner", 
                                  "Two-stage Super Leaner + Indicator", "Discrete two-stage Learner", "Discrete two-stage Leaner + Indicator"),
                    "MAE" = mae.value)
  ...
  Rsq_df <- data.frame("Algo" = c(twostage.fit$libraryNames, "One-stage Super Learner", "Two-stage Super Learner", 
                                  "Two-stage Super Leaner + Indicator", "Discrete two-stage Learner", "Discrete two-stage Leaner + Indicator"),
                    "Rsq" = Rsq.value)
...
# Final results data frame (personal addition)
results_df <- data.frame("Algo" = c(twostage.fit$libraryNames, "One-stage Super Learner", "Two-stage Super Learner", 
                                    "Two-stage Super Leaner + Indicator", "Discrete two-stage Learner", "Discrete two-stage Leaner + Indicator"),
                         "MSE" = mse.value, "Rsq" = Rsq.value, "MAE" = mae.value)

# Creating MSE(10^8), Relative MSE, and Relative Error benchmarks (relative to two-stage SL + indicators) from paper
results_df <- results_df %>%
  mutate(
    MSE_10_8 = MSE / 10^8,
    Relative_MSE = MSE / results_df %>% filter(Algo == "Two-stage Super Leaner + Indicator") %>% pull(MSE),
    RE = Rsq / results_df %>% filter(Algo == "Two-stage Super Leaner + Indicator") %>% pull(Rsq)
  ) %>%
